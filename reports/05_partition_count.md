...
Extra: Using index
```
**考察**: 8パーティションを順次カウント。パーティション数が少ないためオーバーヘッド最小。**並列実行の可能性**あり（MySQL 8.0 の並列クエリ機能）。
...
- **意外な結果**: 1000パーティションが最速（67ms）なのは、パーティション間の並列処理と、各パーティションサイズのバランスが良いため
- **8000パーティション**: オーバーヘッドが並列処理の効果を上回り、8パーティションと同等の遅さに
