...
```
```
partitions: p800,p801,...,p880 (81パーティション)
type: range
key: PRIMARY
rows: 10001
Extra: Using where; Using index
...
**考察**: パーティションキー以外（created_at）での検索は全パーティションスキャン。HASH(944ms) より RANGE(204.8ms) が4.6倍高速だが、両方とも全パーティションスキャンのため理論上は同等のはず。この差の原因は不明であり、測定環境や実行タイミングによる可能性がある。**この結果の再現性は未検証**。
...
**考察**: 8000パーティションが最速という結果は他のベンチマーク傾向と矛盾しており、**測定誤差の可能性が高い**。複数回測定による検証が必要。この結果は参考値として扱い、結論の根拠には含めない。
...
**考察**: EXPLAIN上は同じだが、実行時間は8000パーティションが最速という異常な結果。他のベンチマーク（パーティション数増加で遅延増大）と矛盾するため、**この結果は信頼性が低い**。
...
**考察**: パーティションキー以外での検索は壊滅的。8000パーティションでは約1660msかかる。全パーティションのセカンダリインデックスを順次スキャンするため。
...
**考察**: rows=115 で同じだが、8000パーティション全てのセカンダリインデックスを開いて検索するため、8パーティション(7.8ms)と比較して**212倍遅い(1657.6ms)**。
...
| ID範囲検索（パーティション内） | 1100ms | 11.5ms | **RANGE（96倍高速）** |
| 日付範囲検索 | 944ms | 204.8ms | RANGE（※差の原因は未検証） |
| Full COUNT | 193ms | 127.5ms | （※測定誤差の可能性） |
| author_id検索 | 未測定 | 1657.6ms | RANGE のみ測定 |
...
### 本検証では 8 パーティションが最も高速
- PK検索: 変わらないか高速
- 範囲検索: 最も高速（RANGE でパーティションプルーニング効果大）
- INSERT: 許容範囲
- ストレージ: 最小限のオーバーヘッド
- **注意**: 8, 100, 1000, 8000 のみ検証。16や32などの中間値は未検証。

### HASH vs RANGE の選択
- **PK検索が多く、初回アクセス（コールドキャッシュ）が重要** → HASH（ただしキャッシュ後は差が小さい）
- **範囲検索が多い** → RANGE（パーティションプルーニングが効く）
- **パーティションキー以外の検索が多い** → パーティション数を最小限に抑える（HASH/RANGE問わず）
