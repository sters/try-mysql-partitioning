...
| **HASH(author_id)** | **51µs** | **37.8%削減** |
| RANGE(author_id) | 482µs | 487.8%増加 |
...
### 1M件での RANGE 遅延に関する注意

1M件のテストで RANGE が 487% 遅いという結果は、後述の 10M件テストの結果（RANGE が高速）と矛盾するように見える。

**考えられる原因**:
1. **データ量の閾値**: 1M件ではパーティションのオーバーヘッドが効果を上回るが、10M件ではパーティションプルーニングの効果がオーバーヘッドを上回る
2. **キャッシュ状態の違い**: テスト実行順序やキャッシュの状態が異なる可能性
3. **測定誤差**: µs単位の測定では誤差が大きい可能性

**結論**: 小規模データ（1M件程度）では RANGE パーティションのメリットが薄い。大規模データでのみ効果を発揮する可能性がある。
...
| HASH(author_id) | 14.6ms | 70.2%削減 |
| **RANGE(author_id)** | **11.1ms** | **77.3%削減** |
